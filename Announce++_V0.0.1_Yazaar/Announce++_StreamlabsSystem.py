...
ranks = ['Caster', 'Editor', 'Moderator', 'VIP', 'Regular', 'Subscriber', 'GameWisp Subscriber']
...
def Execute(data):
  if data.GetParam(0).lower() == Settings["Command"].lower():
    ...
    if MyMessage.split(" ")[0] == "-a":
      ...
      for i in MyMessage.split(" ")[0][2:]:
        if i == "0" or i == "1" or i == "2" or i == "3" or i == "4" or i == "5" or i == "6" or i == "7" or i == "8" or i == "9" or i.lower() == "a" or i.lower() == "b" or i.lower() == "c" or i.lower() == "d" or i.lower() == "e" or i.lower() == "f":
          ...
          Parent.SendStreamMessage("Send a valid request, usage: -w/-a/-HEX/-CSS_COLOR_NAME -mute(optional) YOUR MESSAGE")
          return
    ...
    Parent.BroadcastWsEvent("Warnings&AnnouncementsByYazaar",json.dumps({"color": ColorType.lower(), "mute": NoAudio, "type": ActionType, "message": MyMessage, "userType": getPermission(data), "username": Parent.GetDisplayName(data.User)}))
    return

def Tick():
 return

def getPermission(data):
  for i in ranks:
    if Parent.HasPermission(data.User,i,"") == True:
      return i
  return 'Everyone'
