...
# Import Libraries
#---------------------------------------
import codecs, datetime, json, os, random, zipfile, time
...
def Execute(data):
  ...
  if data.GetParam(0).lower() == Settings["Command"] and data.GetParamCount() == 2:
    ...
    try:
      ...
      Parent.SendStreamMessage("/me [PyInstaller] Installing " + str(data.GetParam(1)) + " -> Make sure to reload the scripts (found top right in Scripts tab)")
    except Exception:
